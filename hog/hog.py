...
def make_averaged(fn, num_samples=1000):
    ...
    def averaged_fn(*args):
        ...
        while calls < num_samples:
            summa = summa + fn(*args)
            calls = calls + 1
        
        return summa / num_samples
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    counter = 1
    highest_average = 0
    number_of_dice = counter
    end = "\n"
    result = ""
    
    while counter <= 10:
        average = make_averaged(roll_dice)(counter, dice)
        print(counter, "dice scores", average, "on average")
        if average > highest_average:
            highest_average, number_of_dice = average, counter
        counter = counter + 1
